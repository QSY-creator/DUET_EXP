...
# 从linear_pattern_extractor导入必要的类
from .linear_pattern_extractor import DFT_series_decomp, MultiScaleSeasonMixing, MultiScaleTrendMixing, PastDecomposableMixing
...
class SparseDispatcher(object):
    ...
    def __init__(self, num_experts, gates):
        ...
        # sort experts
        sorted_experts, index_sorted_experts = torch.nonzero(gates).sort(0)#sort(0):对每一列独立排序；sort(1):对每一行独立排序;返回排序好的值（值，值，值）和每一列原位置索引（原索引，原索引，原索引）
        # drop  
        _, self._expert_index = sorted_experts.split(1, dim=1)#取出排序好的专家序列，舍去排序好的样本批号序列[专家0，专家0，专家1，专家2，专家2]
        # 按专家序号排列，self._batch_index是这个专家顺序的排列下用到每个专家的样本批号
        self._batch_index = torch.nonzero(gates)[index_sorted_experts[:, 1], 0]#[样本0,样本2,样本1,样本5,样本0]是对应[专家0，专家0，专家1，专家2，专家3]
        
        self._part_sizes = (gates > 0).sum(0).tolist()#在gates中计算每个专家获得的样本数，.sum(0)表示对每一列求和，.tolist()表示将tensor转化为list
        # expand gates to match with self._batch_index
        gates_exp = gates[self._batch_index.flatten()]#首先展平，确保self._batch_index是一个一维tensor，然后gates[],按照样本号取出对应的行，gates_exp的shape是[样本数，专家数]，每一行是一个样本对应的所有专家的gate值
        self._nonzero_gates = torch.gather(gates_exp, 1, self._expert_index)
        #torch.gather()函数的作用是根据索引从输入张量中提取值，返回一个新的张量。gather()函数的第一个参数是输入张量，第二个参数是索引张量，第三个参数是维度。这里的意思是根据self._expert_index的索引从gates_exp中提取值，返回一个新的张量self._nonzero_gates
        #最终得到的self._nonzero_gates是一个一维tensor，表示每个样本对应的专家的gate值。按照专家序列下的取得门控值。一维。
    def dispatch(self, inp):
        ...
        #.squeeze(1)表示去掉维度为1的维度，inp_exp的shape是[样本数，特征数]，每一行是一个样本对应的所有专家的输入特征
